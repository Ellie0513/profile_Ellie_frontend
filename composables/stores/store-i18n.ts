...
export const StoreI18n = pinia.defineStore('StoreI18n', () => {
  const useLocale = true; // 是否使用語系地區
  const $router = useRouter();
  const locale = useCookie<LocaleType>('i18nLocale', { default: () => 'tw' });
  const localeGroup = ['tw', 'en', 'jp'];
  // 已使用過瀏覽器設定語系地區
  const alreadyUseBrowserLocale = useCookie<Boolean>('useI18nBrowserLocale', { default: () => false });
